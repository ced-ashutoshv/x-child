		...
		if ( 'cancelled' !== $subscription->get_status() ) {
			...
			<select name="new_interval" id="new_interval">
				...
				foreach ( $intervals as $interval => $label ) {
					?>
					<option value="<?php echo esc_attr( $interval ); ?>" <?php selected( $subscription->get_billing_interval(), $interval ); ?>><?php echo esc_html( $label ) . ' days'; ?></option>
					<?php
				}
				?>
			</select>
			<input type="hidden" value="month" name="new_period" id="new_period">
			<?php
